...
final class GameServer
{
    ...
    private function findOrCreateRoom(string $base,string $requested,array $map): RoomState
    {
        ...
        for($n=1;$n<1000;$n++){
            $name=$base.'-'.$n;
            foreach($this->rooms as $r)if(strcasecmp($r->name,$name)===0&&count($r->clients)<(int)$map['MaxPlayers'])return $r;
        }
    ...
    private function sendMoveToArea(ClientSession $c,RoomState $r): void
    {
        ...
        $o['mapArrows']=$this->world->mapArrowPayload((int)$r->map['id']);
    ...
    private function handleConsoleCommand(string $command): array
    {
        ...
        if($name==='help'){
            return ['ok'=>true,'message'=>'Commands: status, players, rooms, say <message>, reload, safe-shutdown [seconds], cancel-shutdown, restart, stop'];
        }
        if($name==='status'){
