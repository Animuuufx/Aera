...
function swfInfo(string $file): array {
 ...
 $p=(int)ceil((5+4*(ord($s[0])>>3))/8)+4;$labels=[];$symbols=[];$as3=false;
 ...
 if($tag===69)$as3=(unpack('V',$d)[1]&8)!==0; if($tag===43)$labels[]=strtok($d,"\0");
 ...
 if($tag===0)break;}
 return ['as3'=>$as3,'labels'=>$labels,'symbols'=>array_values($symbols)];
}
