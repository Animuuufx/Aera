...
import flash.system.ApplicationDomain;
    ...
    public class Game extends MovieClip 
    {
        ...
        public static var serverPort:int = 5589;
        ...
        // Numeric key codes keep this compatible with older Flash/AIR targets:
        ...
        // Legacy AQW map compatibility. Some older map SWFs (including town)
        // read this public Game property even when WildTangent promotions are
        // not used by Aera. Keep it present and false so those frame scripts
        // do not throw Error #1069.
        ...
        public var userLogoutPending:Boolean = false;
        ...
        // No-op legacy WildTangent hooks retained for old map SWFs.
        public function WT_init():void
        {
            isWTPromoAvailable = false;
        }

        public function WT_checkForPromo():void
        {
            isWTPromoAvailable = false;
        }
        ...
        {
            // Aera does not use WildTangent promotions.
        }

        public function WT_trace():void
        {
            // Compatibility stub for legacy map scripts.
        }
        ...
        public function Game()
        {
            ...
            trace("[Aera v30.75] client loaded; dash stamina cost=50");
            ...
            var onConnectionLostHandler:Function = function (_arg_1:SFSEvent)
            {
                ...
                if (!serialCmdMode)
                {
                    ...
                    if ((!userLogoutPending) && (_arg_1.params != null))
                    {
                ...
                if (protocol == "str")
                {
                    ...
                    switch (cmd)
                    {
                        ...
                        case "popup":
                            ...
                            break;
                        case "iay":
                            // Aera v30.61 dedicated staff announcement packet:
                            // ["iay", rank, username, message].  Do not route this
                            // through the moderator channel; that loses the real rank.
                            str = String(resObj[2]);
                            unm = String(resObj[3]);
                            msg = String(resObj[4]);
                            msg = chatF.cleanChars(msg);
                            msg = chatF.cleanStr(msg);
                            if (((chatF != null) && ("pushAeraIAY" in chatF)))
                            {
                                chatF.pushAeraIAY(str, unm, msg);
                            }
                            else
                            {
                                chatF.pushMsg("server", (("[" + str) + "] Server: (" + rootClass.strToProperCase(unm) + "): " + msg), "SERVER", "", 0);
                            }
                            break;
                        case "moderator":
                            typ = "moderator";
                ...
                if (protocol == "json")
                {
                    ...
                    switch (cmd)
                    {
                        ...
                        case "moveToArea":
                            ...
                            // Database-driven NPC payload (Aera v30.20).
                            // These arrays mirror the legacy AQW private-server protocol:
                            // npcBranch = runtime state, npcdef = definitions, npcmap = placement/appearance/buttons.
                            ...
                            // Database-driven room/map transition arrows (Aera v30.38).
...
//                            showPortraitBox(world.myAvatar, ui.mcPortrait);
                            ...
                            {
                                world.myAvatar.objData.statPoints = int(resObj.StatPoints);
                            }
                            if (((resObj.StatPointsAwarded != null) && (int(resObj.StatPointsAwarded) > 0)))
                            {
                                chatF.pushMsg("server", (("You gained " + int(resObj.StatPointsAwarded)) + " Stat Points!"), "SERVER", "", 0);
                            }
                        ...
                        case "stu":
                            ...
                            // Aera v30.75: the live stat packet can carry/rebuild the legacy
                            // dash-cost field. Force it back to 50 every time stats are refreshed
                            // so a class/stat update cannot silently restore a 100-stamina dash.
                            if (uoLeaf.sta == null)
                            {
                                uoLeaf.sta = {};
                            }
                            uoLeaf.sta["$dsh"] = 50;
                            if (((avt != null) && (avt.dataLeaf != null)))
                            {
                                if (avt.dataLeaf.sta == null)
                                {
                                    avt.dataLeaf.sta = {};
                                }
                                avt.dataLeaf.sta["$dsh"] = 50;
                            }
                            if (resObj.statPoints != null)
                            {
                                world.myAvatar.objData.statPoints = int(resObj.statPoints);
                            }
                            if (resObj.allocatedStats != null)
                            {
                                world.myAvatar.objData.allocatedStats = resObj.allocatedStats;
                            }
                        ...
                        case "spendStatPoints":
                            if (resObj.bitSuccess == 1)
                            {
                                world.myAvatar.objData.statPoints = int(resObj.Points);
                                if (resObj.allocatedStats != null)
                                {
                                    world.myAvatar.objData.allocatedStats = resObj.allocatedStats;
                                }
                            ...
                            {
                                MsgBox.notify((resObj.msg != null) ? resObj.msg : "Unable to spend stat point.");
                            }
                            break;
                        case "saveStatPoints":
                            if (resObj.bitSuccess == 1)
                            {
                                ...
                                if (mcStatsPanel)
                                {
                                    mcStatsPanel.onStatAllocationSaved();
                                    mcStatsPanel.update();
                                }
                            }
                            else
                            {
                                if (resObj.Points != null)
                                {
                                    world.myAvatar.objData.statPoints = int(resObj.Points);
                                }
                                if (resObj.allocatedStats != null)
                                ...
                                if (mcStatsPanel)
                                {
                                    mcStatsPanel.onStatAllocationFailed();
                                    mcStatsPanel.update();
                                }
                                MsgBox.notify((resObj.msg != null) ? resObj.msg : "Unable to save stat points.");
                            }
                            break;
                        case "resetStatPoints":
                            if (resObj.bitSuccess == 1)
                            {
                                ...
                                if (mcStatsPanel)
                                {
                                    mcStatsPanel.onStatAllocationReset();
                                    mcStatsPanel.update();
                                }
                            }
                            else
                            {
                                ...
                                MsgBox.notify((resObj.msg != null) ? resObj.msg : "Unable to reset stat points.");
                            }
                            break;
                        case "event":
...
//            Security.allowDomain("*.aq.com");
        ...
        {
            var loader:URLLoader = new URLLoader(new URLRequest(getFilePath() + file));
            loader.dataFormat = URLLoaderDataFormat.BINARY;

            if (onComplete != null) loader.addEventListener(Event.COMPLETE, this.onLoadToBytes(onComplete, context));
            if (onProgress != null) loader.addEventListener(ProgressEvent.PROGRESS, onProgress);
...
//            csLoader.removeEventListener(IOErrorEvent.IO_ERROR, onCSError);
        ...
        private function keyboardMovementFrame(event:Event):void
        {
            ...
            // Feed held direction every frame. World only creates a new long
            // walk target when direction changes or the current target is close,
            // so AvatarMC.walkTo() is no longer restarted every ~85 ms.
        ...
        public function key_StageGame(_arg_1:KeyboardEvent):*
        {
            ...
            {
                // ESC closes the database NPC dialog first.
                if (
                ...
                // WASD movement is held-key movement.
                // E opens the closest database NPC when the player is nearby.
                ...
                if (_arg_1.keyCode == litePreference.data.keys["Dash"])
                {
                    if ((((((ui) && (ui.mcInterface)) && (ui.mcInterface.te)) && (!(_arg_1.target == ui.mcInterface.te))) && (!(_arg_1.target == ui.mcInterface.ncText))))
                    {
                        // Aera v30.75: set the actual data leaf AND the uoTree copy.
                        // The original dash bytecode consumes sta.$dsh, so both locations must
                        // be corrected before the movement click occurs.
                        if (world.myAvatar.dataLeaf.sta == null)
                        {
                            world.myAvatar.dataLeaf.sta = {};
                        }
                        world.myAvatar.dataLeaf.sta["$dsh"] = 50;
                        try
                        {
                            if (world.uoTree[world.myAvatar.pnm] != null)
                            {
                                if (world.uoTree[world.myAvatar.pnm].sta == null)
                                {
                                    world.uoTree[world.myAvatar.pnm].sta = {};
                                }
                                world.uoTree[world.myAvatar.pnm].sta["$dsh"] = 50;
                            }
                        }
                        catch(dashKeyErr:Error)
                        {
                        }
                        if (world.myAvatar.dataLeaf.intSP >= 50)
                        {
                            pDash = true;
                        }
                    }
        ...
        public function initLogin():void
        {
            ...
            if ((("logoutWarning" in userPreference.data) && (String(userPreference.data.logoutWarning).length > 0)))
            {
                mcLogin.warning.s = String((String(userPreference.data.logoutWarning) + "\nYou will be able to login after $s seconds."));
            }
        ...
        public function login(strUsername:String, strPassword:String):*
        {
            ...
            {
                mcConnDetail.showError("Login connection failed. " + error.message);
            }
        }
        ...
        {
            var detail:String = "";
            if (_arg_1 is IOErrorEvent)
            {
                detail = IOErrorEvent(_arg_1).text;
            }
            else if (_arg_1 is SecurityErrorEvent)
            {
                detail = SecurityErrorEvent(_arg_1).text;
            }
            trace("Login transport error: " + detail);
            mcConnDetail.showError("Could not reach the Aera login server." + ((detail.length > 0) ? ("\n" + detail) : ""));
        }
        ...
        public function connectTo(_arg_1:String, _arg_2:int=5589):*
        {
            serverIP = _arg_1;
...
//            _local_2.load(new URLRequest(((serverGamePath + "api/game/clientvars?v=") + Math.random())));
        ...
        public function onInfoComplete(_arg_1:Event):void
        {
            ...
            iMaxLoadoutSlots = Number(_local_2["iMaxLoadoutSlots"]);
