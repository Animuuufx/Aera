package liteAssets
{
    ...
    public class RiftSpawnPoint
    {
        ...
        public static function choose(map:DisplayObjectContainer,space:DisplayObjectContainer,seed:uint):Point
        {
            ...
            var random:Function=function():Number { state^=state<<13;state^=state>>>17;state^=state<<5;return Number(state)/4294967296; };
            for(var attempt:int=0;attempt<256;attempt++){
                var surface:DisplayObject=surfaces[int(random()*surfaces.length)];
                var bounds:Rectangle=surface.getBounds(space);
                var point:Point=new Point(bounds.x+random()*bounds.width,bounds.y+random()*bounds.height);
