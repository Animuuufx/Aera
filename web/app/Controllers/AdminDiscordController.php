...
final class AdminDiscordController
{
    ...
    private function requestBotShutdown(): array
    {
        ...
        stream_set_timeout($fp,1);$request="POST /shutdown HTTP/1.1\r\nHost: 127.0.0.1\r\nX-Aera-Shutdown-Token: ".$token."\r\nConnection: close\r\nContent-Length: 0\r\n\r\n";fwrite($fp,$request);$response='';while(!feof($fp)){$chunk=fread($fp,4096);if($chunk===false||$chunk==='')break;$response.=$chunk;}fclose($fp);$parts=preg_split("/\r\n\r\n/",$response,2);$code=0;if(isset($parts[0])&&preg_match('/^HTTP\/\S+\s+(\d+)/m',$parts[0],$m))$code=(int)$m[1];if($code!==202)return ['ok'=>false,'running'=>true,'message'=>'Discord bot rejected the shutdown request.'];return ['ok'=>true,'queued'=>true,'action'=>'stop','running'=>true,'message'=>'Discord bot shutdown requested.'];
    }
