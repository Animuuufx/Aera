...
function Get-BotProcess { if(!(Test-Path $PidFile)){return $null};try{$pid=[int](Get-Content $PidFile -Raw).Trim();return Get-Process -Id $pid -ErrorAction Stop}catch{Remove-Item $PidFile -Force -ErrorAction SilentlyContinue;return $null} }
function Write-Result($ok,$message,$pid=$null){[pscustomobject]@{ok=$ok;message=$message;pid=$pid;running=[bool](Get-BotProcess)}|ConvertTo-Json -Compress}
...
'start'{if(!(Test-Path $EnvFile)){Write-Result $false 'DiscordBot/.env is not configured. Save the bot settings from the admin panel first.';exit 1};$existing=Get-BotProcess;if($existing){Write-Result $true 'Discord bot is already running.' $existing.Id;exit 0};$node=(Get-Command node -ErrorAction SilentlyContinue).Source;if(!$node){Write-Result $false 'Node.js was not found in PATH. Install Node.js 20+ on the server.';exit 1};if(!(Test-Path (Join-Path $Root 'node_modules'))){$npm=(Get-Command npm -ErrorAction SilentlyContinue).Source;if(!$npm){Write-Result $false 'npm was not found in PATH.';exit 1};&$npm --prefix $Root install --omit=dev|Out-Null;if($LASTEXITCODE-ne 0){Write-Result $false 'npm install failed.';exit 1}};Remove-Item $LogFile,$ErrorLog -Force -ErrorAction SilentlyContinue;$p=Start-Process -FilePath $node -ArgumentList @('src/index.js') -WorkingDirectory $Root -RedirectStandardOutput $LogFile -RedirectStandardError $ErrorLog -PassThru -WindowStyle Hidden;Set-Content -Path $PidFile -Value $p.Id -NoNewline;Start-Sleep -Milliseconds 700;$check=Get-BotProcess;if(!$check){Write-Result $false 'Discord bot exited immediately. Check discord-bot.log and discord-bot-error.log.';exit 1};Write-Result $true 'Discord bot started.' $check.Id;exit 0}
'restart'{&$MyInvocation.MyCommand.Path -Action stop|Out-Null;&$MyInvocation.MyCommand.Path -Action start;exit $LASTEXITCODE}
'deploy'{if(!(Test-Path $EnvFile)){Write-Result $false 'DiscordBot/.env is not configured.';exit 1};$npm=(Get-Command npm -ErrorAction SilentlyContinue).Source;if(!$npm){Write-Result $false 'npm was not found in PATH.';exit 1};&$npm --prefix $Root install --omit=dev|Out-Null;if($LASTEXITCODE-ne 0){Write-Result $false 'npm install failed.';exit 1};&$npm --prefix $Root run deploy;if($LASTEXITCODE-ne 0){Write-Result $false 'Discord slash-command deployment failed.';exit 1};Write-Result $true 'Discord slash commands deployed.';exit 0}
