...
// The Aera server tracks the aggregate online player count in servers.count; users.online does not exist.
async function getOnlineCount() {
  const [rows] = await pool.query(
    'SELECT COALESCE(SUM(CAST(`count` AS UNSIGNED)), 0) AS count FROM `servers`'
  );
  ...
  try {
    switch (interaction.commandName) {
      case 'server': {
        ...
        await interaction.reply({
          embeds: [new EmbedBuilder()
            .setTitle('Aera Server')
            .setDescription('The Aera game server is online.')
            .addFields({ name: 'Players Online', value: `**${online}**`, inline: true })
            .setTimestamp()]
        });
        break;
      }
      ...
      case 'player': {
        ...
        if (!player) {
          await interaction.reply({ content: `No Aera player matching **${search}** was found.`, ephemeral: true });
          break;
        }
        const status = player.player_online === null || player.player_online === undefined
          ? 'Unknown'
          : Number(player.player_online) === 1 ? 'Online' : 'Offline';
        const level = player.player_level === null || player.player_level === undefined
          ? 'Unknown'
          : String(player.player_level);
        await interaction.reply({
          embeds: [new EmbedBuilder()
            .setTitle(`Player: ${player.player_name}`)
            .addFields(
      ...
      case 'kick': {
        ...
        if (!member.kickable) {
          await interaction.reply({ content: 'I cannot kick that member. Check the bot role hierarchy.', ephemeral: true });
          break;
        }
      ...
      case 'ban': {
        ...
        if (!member.bannable) {
          await interaction.reply({ content: 'I cannot ban that member. Check the bot role hierarchy.', ephemeral: true });
          break;
        }
      ...
      case 'timeout': {
        ...
        if (!member.moderatable) {
          await interaction.reply({ content: 'I cannot timeout that member. Check the bot role hierarchy.', ephemeral: true });
          break;
        }
        await member.timeout(minutes * 60 * 1000, reason);
        await interaction.reply(`Timed out **${member.user.tag}** for **${minutes} minutes**. Reason: ${reason}`);
        break;
      }
      case 'clear': {
        if (!(await requireModerator(interaction))) break;
    ...
    const message = error?.code === 'ER_NO_SUCH_TABLE'
      ? 'The required Aera database table is unavailable.'
...
function writeHeartbeat() {
  try {
    fs.writeFileSync(
      heartbeatFile,
      JSON.stringify({ pid: process.pid, ok: true, discordReady: client.isReady(), timestamp: new Date().toISOString() }),
      'utf8'
    );
  } catch (error) {
    console.error('[Aera Discord] Could not write health heartbeat:', error.message);
  }
}
