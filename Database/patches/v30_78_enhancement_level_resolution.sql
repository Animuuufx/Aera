...
-- 4) Canonicalize every generated enhancement ITEM to the definition row that
-- matches the item's actual Level and enhancement family. This is important for
-- catalogs such as 900100 where EnhID historically remained 1 while Level=100.
-- Existing valid definitions are preferred and missing rows are left for runtime
-- fallback handling in v30.77; this statement never changes normal non-enhancement
-- items.
UPDATE `items` i
JOIN `enhancements` e
  ON e.`Level`=i.`Level`
